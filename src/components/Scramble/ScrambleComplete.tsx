...
const ScrambleComplete = ({ scramble }: Props) => {
  return (
    <div
      className={`flex flex-col justify-center gap-5 text-center ${pm.className} bg-basketball-scene bg-cover bg-center h-screen w-full`}
    >
      ...
      {scramble.map((item, index) => {
        return (
          <div
            key={index}
            className="text-4xl text-slate-950 mt-6 drop-shadow-2xl"
          >
            {item.word}
          </div>
        );
